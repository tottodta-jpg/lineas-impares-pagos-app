...
export default function App() {
  ...
  // Inicializa el modo oscuro desde localStorage o sistema
  ...
  // Maneja el cambio de tema aplicando la clase 'dark' al HTML
  useEffect(() => {
    const root = window.document.documentElement;
    if (isDarkMode) {
      root.classList.add('dark');
      localStorage.setItem('theme_modo', 'dark');
    } else {
      root.classList.remove('dark');
      localStorage.setItem('theme_modo', 'light');
    }
  }, [isDarkMode]);
  ...
  if (!isAuthenticated) {
    return (
      // Se removió el div externo con className={isDarkMode ? 'dark' : ''} 
      // porque el modo oscuro ahora se controla en el nivel <html>
